...
void receiveCameraTxData(const byte *buf, size_t size){
  ...
  //if (size != sizeof(payload)) return;
  memcpy(&payload, buf, sizeof(payload));
  sensorvalues.bluegoal_relativeposition.angle = payload.cameraTxData.values[0];
  sensorvalues.bluegoal_relativeposition.distance = payload.cameraTxData.values[1];
  sensorvalues.yellowgoal_relativeposition.angle = payload.cameraTxData.values[2];
  sensorvalues.yellowgoal_relativeposition.distance = payload.cameraTxData.values[3];
  sensorvalues.ball_relativeposition.angle = payload.cameraTxData.values[4];
  sensorvalues.ball_relativeposition.distance = payload.cameraTxData.values[5];
  return;
}
...
void getBNOreading(){
     ...
     if (bno.getSensorEvent() == true) {
          if (bno.getSensorEventID() == SENSOR_REPORTID_GYRO_INTEGRATED_ROTATION_VECTOR) {
               sensorvalues.relativeBearing = (bno.getGyroIntegratedRVK()) * 180.0 ; // Convert yaw / heading to degree
          }
     }
}
